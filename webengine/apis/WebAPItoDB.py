try:
    ...
    cnx = mysql.connector.connect(host=MySQLconfig.MYSQL_HOST,user=MySQLconfig.MYSQL_USER,
                                        passwd=MySQLconfig.MYSQL_PASSWORD,database=MySQLconfig.MYSQL_DATABASE)
    cursor = cnx.cursor()
    ...
    genders = ["Male","Female"]
    gmembers = api.get_guild_profile('eu', 'aggramar', 'fury',locale='en_GB', fields='members')
    ...
    for x in c_sql:
        if x not in c_api:
            print('\nMostly reliable sources told me',x,'overstayed his/her welcome in our table!')
    ...
    for x in (gmembers['members']):
        ...
        try:
            wowcharacter = api.get_character_profile('eu',wow_realm, wow_character, fields='items,guild')
            race_name = races[wowcharacter['race']]
            class_name = classes[wowcharacter['class'] - 1]
            gender_name = genders[wowcharacter['gender']]
            print('\nInserting:', wowcharacter['name'],class_name,race_name,gender_name,wowcharacter['level'],wowcharacter['items']['averageItemLevelEquipped'],
                wowcharacter['guild']['name'],wow_realm,wowcharacter['totalHonorableKills'],'into database.')
            ...
            val = (wowcharacter['name'], class_name, race_name, gender_name, wowcharacter['level'], wowcharacter['items']['averageItemLevelEquipped'],
                wowcharacter['guild']['name'], wow_realm,wowcharacter['totalHonorableKills'], wowcharacter['thumbnail'])
            cursor.execute(sql, val)
            cnx.commit()
            print (cursor.rowcount,'records inserted/updated.')
        except:
            print('\nCharacter',wow_character,'not found on',wow_realm,'in armory or not actually in guild!')
        finally:
            index_count +=1
    print('\nDatabase updates complete!\n')
    cnx.close()
except:
    print('Something went wrong during update from API to database, API or database could be down...')